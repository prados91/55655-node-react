...
export const CartProvider = ({ children }) => {
    ...
    const deleteAll = async (oid) => {
        try {
            const user = await verifyUser()
            if (user !== null) {
                let cookie = document.cookie.split("; ")
                cookie = cookie.find(each => each.split("=")[0] === "token")
                const res = await axios.delete(`https://coderbasketstore.up.railway.app/api/orders/delete/all`, cookie)
                if (res.data.statusCode === 200) {
                    Swal.fire({
                        title: `Orders eliminated`,
                        icon: "warning",
                        confirmButtonColor: "#3085d6",
                        confirmButtonText: "OK",
                    }).then(() => {
                    ...
                    Swal.fire({
                        title: `Sorry, we can't delete the orders`,
                        icon: "error",
                        confirmButtonColor: "#3085d6",
                        confirmButtonText: "OK",
                    }).then(() => {
    ...
    const report = async () => {
        try {
            ...
            if (user !== null) {
                const { _id } = user
                //const res = await axios.get(`https://coderbasketstore.up.railway.app/api/orders/total/${_id}`)
                const res = await axios.get(`https://coderbasketstore.up.railway.app/api/orders/tickets`)
    ...
    return (
        <CartContext.Provider value={{ cart, readCart, createOrder, deleteOrder, deleteAll, updateOrder, addItem, checkout, report }}>
            {children}
